...
import { B as S, P as g } from "./password-input-BeMH8qwj.js";
import { C as A } from "./chevron-down-qwNovDle.js";
/**
...
function $({ password: t }) {
  if (!t) return null;
  const { label: s, score: a, color: l } = M(t);
  return e.jsxs("div", {
    className: "mt-2",
    children: [
      e.jsx("div", {
        ...
        children: [1, 2, 3].map((r) =>
          e.jsx(
            "div",
            {
              className: `h-1 flex-1 rounded-full transition-all duration-300 ${r <= a ? l : "bg-slate-200"}`,
            },
            r,
          ),
        ),
      ...
      e.jsx("p", {
        className: `text-xs font-medium ${a === 1 ? "text-red-500" : a === 2 ? "text-amber-500" : "text-emerald-600"}`,
        children: s,
      }),
    ],
  });
...
function F({ value: t, onChange: s, error: a }) {
      ...
      e.jsxs("div", {
        ...
        children: [
          ...
          e.jsxs("button", {
            ...
            "aria-expanded": l,
            "aria-required": "true",
            className: `relative z-20 w-full flex items-center justify-between px-3 py-2.5 text-sm bg-white border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 transition-shadow ${a ? "border-red-400 focus:ring-red-400" : "border-slate-200 hover:border-slate-300"} ${t ? "text-slate-900" : "text-slate-400"}`,
            children: [
...
const O = "http://localhost:3000/api/v1";
function R(t) {
  const s = {};
  return (
    t.name.trim().length < 2 && (s.name = "Name must be at least 2 characters"),
    t.email
      ? /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t.email) ||
        (s.email = "Enter a valid email address")
      : (s.email = "Email is required"),
    t.password.length < 6 &&
      (s.password = "Password must be at least 6 characters"),
    t.confirmPassword
      ? t.password !== t.confirmPassword &&
        (s.confirmPassword = "Passwords do not match")
      : (s.confirmPassword = "Please confirm your password"),
    t.role || (s.role = "Please select a role"),
    s
  );
}
function W() {
  ...
  async function w(n) {
    ...
    try {
      ...
      (t(x.name, x.email, x.role, v, !0), s("/", { replace: !0 }));
    } catch {
      o({
...
function j({ label: t, error: s, htmlFor: a, children: l }) {
  return e.jsxs("div", {
    children: [
      e.jsxs("label", {
        htmlFor: a,
