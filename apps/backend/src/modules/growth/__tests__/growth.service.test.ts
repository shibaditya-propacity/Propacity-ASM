...
describe("GrowthService", () => {
  ...
  describe("updateProspectStage", () => {
    it("throws WorkshopNotFoundError if prospect not found", async () => {
      ...
      await expect(
        service.updateProspectStage("tenant-1", user, "prospect-1", {
          stage: "Attended",
        }),
      ).rejects.toThrow();
    });
    ...
    it("throws InvalidStageTransitionError for illegal transition", async () => {
      ...
      await expect(
        service.updateProspectStage("tenant-1", user, "p1", {
          stage: "Registered",
        }),
    ...
    it("updates stage when transition is valid", async () => {
      ...
      mockRepo.updateProspectStage.mockResolvedValue({
        ...prospect,
        stage: "Attended",
      });
      mockRepo.createProspectActivity.mockResolvedValue({
        id: "act-1",
        tenantId: "t1",
        prospectId: "p1",
        actorId: user.id,
      ...
      const result = await service.updateProspectStage("t1", user, "p1", {
        stage: "Attended",
      });
