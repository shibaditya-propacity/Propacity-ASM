...
describe("IntegrationsService", () => {
  ...
  describe("connectApiKey", () => {
    it("connects successfully for an API_KEY provider", async () => {
      ...
      const result = await service.connectApiKey("tenant-1", "prov-1", {
        apiKey: "sk-test-123",
    ...
    it("throws ProviderNotFoundError for unknown provider", async () => {
      ...
      await expect(
        service.connectApiKey("tenant-1", "unknown", { apiKey: "sk-test" }),
  ...
  describe("disconnect", () => {
    it("deletes the integration when it exists", async () => {
      ...
      await service.disconnect("tenant-1", "prov-1");

      expect(repo.deleteIntegration).toHaveBeenCalledWith("tenant-1", "prov-1");
    });
    ...
    it("throws IntegrationNotFoundError when integration does not exist", async () => {
      ...
      await expect(service.disconnect("tenant-1", "prov-1")).rejects.toThrow(
        IntegrationNotFoundError,
      );
    });
  });
  ...
  describe("getModuleReadiness", () => {
    it("returns readiness for all modules", async () => {
      ...
      const result = await service.getModuleReadiness("tenant-1");
